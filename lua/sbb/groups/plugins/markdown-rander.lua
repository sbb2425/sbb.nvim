...
function M.get(opts, c)
  return {
    ...
    RenderMarkdownCode = { bg = opts.transparent and c.ui.bg.none or c.ui.bg.soft }, -- фон блока кода
    RenderMarkdownCodeInfo = { fg = c.ui.bg.light, bg = opts.transparent and c.ui.bg.none or c.ui.bg.default }, -- секция инфо после языка в code fence
    RenderMarkdownCodeBorder = { fg = c.ui.bg.light, bg = opts.transparent and c.ui.bg.none or c.ui.bg.default }, -- фон границы блока кода
    RenderMarkdownCodeFallback = { fg = c.ui.bg.light, bg = opts.transparent and c.ui.bg.none or c.ui.bg.default }, -- фолбэк-цвет языка, если у провайдера иконок нет значения
    RenderMarkdownCodeInline = { fg = c.ui.bg.light, bg = opts.transparent and c.ui.bg.none or c.ui.bg.default }, -- фон инлайн-кода
    ...
    RenderMarkdownTableHead = { fg = c.accent.blue.default, bg = opts.transparent and c.ui.bg.none or c.ui.bg.soft }, -- строки заголовка таблицы (pipe table)
    RenderMarkdownTableRow = { fg = c.ui.fg.muted, bg = opts.transparent and c.ui.bg.none or c.ui.bg.soft }, -- остальные строки таблицы
    RenderMarkdownSuccess = { fg = c.accent.green.bright, bg = opts.transparent and c.ui.bg.none or c.ui.bg.soft }, -- callout'ы категории "success" (Tip, Success, Done, Check, Hint...)
    RenderMarkdownHint = { fg = c.accent.cyan.bright, bg = opts.transparent and c.ui.bg.none or c.ui.bg.soft }, -- callout'ы категории "hint" (Important, Example)
    RenderMarkdownWarn = { fg = c.accent.orange.bright, bg = opts.transparent and c.ui.bg.none or c.ui.bg.soft }, -- callout'ы категории "warning" (Warning, Question, Help, Faq, Attention)
    RenderMarkdownInfo = { fg = c.accent.purple.bright, bg = opts.transparent and c.ui.bg.none or c.ui.bg.soft }, -- callout'ы категории "info" (Note, Abstract, Summary, Tldr, Info, Todo)
    RenderMarkdownError = { fg = c.accent.red.bright, bg = opts.transparent and c.ui.bg.none or c.ui.bg.soft }, -- callout'ы категории "error" (Caution, Failure, Missing, Danger, Error, Bug)
    RenderMarkdownQuote = { fg = c.accent.purple.bright, bg = opts.transparent and c.ui.bg.none or c.ui.bg.soft }, -- дефолт для блочных цитат / Quote и Cite callout'ов
